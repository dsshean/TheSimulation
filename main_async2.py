# main_async2.py - Combined Simulation Logic (Toy Example Structure)
...
# --- Project Imports (Simulated - Functions defined below) ---
# from src.config import settings # Replaced by direct config vars
# from src.initialization import ... # Functions integrated below
# from src.tasks_async import ... # Functions integrated below

# --- Logging Setup ---
log_filename = "main_async2.log"
logging.basicConfig(
...
logger.info("--- Starting Combined Async Simulation (main_async2.py) ---")
...
APP_NAME = "TheSimulationAsyncCombined"
USER_ID = "player1"
...
# --- Core Components (Global Scope - Like Toy Example) ---
...
# --- ADK Components (Global Scope - Like Toy Example) ---
adk_session_service: Optional[InMemorySessionService] = None
adk_session_id: Optional[str] = None # Will be set after state load
...
# --- State Keys (Copied from initialization.py) ---
...
# --- Initialization Functions (Integrated from initialization.py) ---
...
# --- Helper Functions (Integrated from tasks_async.py) ---
...
# --- Pydantic Models (Integrated from tasks_async.py) ---
...
# --- Agent Definitions (Using Prompts from tasks_async.py) ---
...
def create_simulacra_llm_agent(sim_id: str, persona_name: str) -> LlmAgent:
    ...
    # --- RESTORED DETAILED STATIC INSTRUCTION ---
    instruction = f"""
...
# --- Task Functions (Integrated from tasks_async.py) ---

def generate_table() -> Table:
    """Generates the Rich table for live display based on the global state."""
    ...
    for i, (obj_id, obj_state_data) in enumerate(objects_dict.items()): # Renamed obj_state to obj_state_data
         if i >= object_limit:
...
async def time_manager_task(live_display: Live):
    ...
    # Access global state and schedule
        ...
        while state.get("world_time", 0.0) < MAX_SIMULATION_TIME:
            ...
            while schedule and schedule[0][0] <= new_sim_time:
                ...
                for key_path, value in results.items():
                    success = _update_state_value(state, key_path, value) # Pass global state
                ...
                if actor_id and "simulacra" in state:
                    actor_sim_state = state["simulacra"].get(actor_id) # Renamed actor_state to avoid conflict
                    if actor_sim_state:
                        if actor_sim_state.get("status") == "busy" and abs(get_nested(actor_sim_state, "current_action_end_time", default=-1) - completion_time) < 0.01:
                            actor_sim_state["status"] = "idle"
                            actor_sim_state["current_action_end_time"] = completion_time
                            ...
                            logger.warning(f"[TimeManager] Action completion for {actor_id} at {completion_time:.1f} did not match current busy state end time {get_nested(actor_sim_state, 'current_action_end_time', default=-1)}. Status left as busy.")
                        else:
            ...
            live_display.update(generate_table()) # Pass no args, uses global state
...
async def world_engine_task_llm():
    ...
    global schedule_event_counter # Need global to modify counter
    # Access global components
    logger.info("[WorldEngineLLM] Task started.")

    if not world_engine_runner:
        logger.error("[WorldEngineLLM] Global runner not initialized. Task cannot proceed.")
        return
    if not world_engine_agent:
        logger.error("[WorldEngineLLM] Global agent instance not initialized. Task cannot proceed.")
        return
    if not adk_session:
        logger.error("[WorldEngineLLM] Global ADK session not initialized. Task cannot proceed.")
        return

    session_id_to_use = adk_session.id # Get ID from global session

    while True:
        intent_event = None
        actor_id = None
        actor_state_we = {} # Renamed actor_state to avoid conflict
        ...
        try:
            ...
            if get_nested(intent_event, "type") != "intent_declared":
                ...
                event_bus.task_done() # Mark ignored task as done
                continue
            ...
            # --- Handle other actions via LLM ---
                ...
                location_state_data = get_nested(state, 'location_details', actor_location_id, default={}) # Renamed
                ...
                # --- TRIGGER TEXT (Context Only - like toy example) ---
                prompt = f"""
...
"""
                ...
                world_engine_runner.agent = world_engine_agent # Set agent before call
                response_text = ""
                trigger_content = genai_types.Content(parts=[genai_types.Part(text=prompt)]) # Pass the context prompt

                async for event in world_engine_runner.run_async(
                    user_id=USER_ID,
...
async def simulacra_agent_task_llm(agent_id: str):
    ...
    if not world_engine_runner: # Use the single global runner
        logger.error(f"[{agent_id}] Global runner not initialized. Task cannot proceed.")
        return
    if not simulacra_agent:
        logger.error(f"[{agent_id}] Global agent instance not found for {agent_id}. Task cannot proceed.")
        return
    if not adk_session:
        logger.error(f"[{agent_id}] Global ADK session not initialized. Task cannot proceed.")
        return

    session_id_to_use = adk_session.id # Get ID from global session
    simulacra_runner = world_engine_runner # Use the single global runner instance

    while True:
        agent_state_sim = None # Renamed agent_state
        try:
            ...
            agent_name = agent_state_sim.get("name", agent_id) # Get name here

            while agent_state_sim.get("status") == "busy":
                await asyncio.sleep(0.5 + random.uniform(0, 0.5))
                agent_state_sim = get_nested(state, "simulacra", agent_id) # Re-fetch state
                if not agent_state_sim: break
            if not agent_state_sim: break # Exit if state disappeared
...
"""
            ...
            if adk_session_service: # Check global service
                logger.debug(f"[{agent_name}] Global Service ID: {id(adk_session_service)}")
            if adk_session_service and hasattr(adk_session_service, '_sessions'):
                 logger.debug(f"[{agent_name}] Sessions in Global Service: {list(adk_session_service._sessions.keys())}")
            ...
            async for event in simulacra_runner.run_async(
                user_id=USER_ID,
                session_id=session_id_to_use,
                new_message=trigger_content
            ):
            ...
            if response_text:
                try:
                   ...
                   console.print(f"[cyan][{agent_name} Thought][/] {validated_intent.internal_monologue}")
                   intent = {
                       "action_type": validated_intent.action_type,
                       "target_id": validated_intent.target_id,
                       "details": validated_intent.details
                   }
                   console.print(f"[yellow][{agent_name} Intent][/] {intent}")
...
async def run_simulation_main():
    """Sets up ADK and runs all concurrent tasks."""
    global adk_session_service, adk_session_id, adk_session, world_engine_runner
    global world_engine_agent, simulacra_agents, state # Ensure state is global

    console.rule("[bold green]Starting Combined Async Simulation[/]")
    ...
    # --- Load State (Integrated from initialize_simulation) ---
    ...
    # For this combined script, let's default to loading the latest
    instance_uuid_arg = None # Or parse sys.argv if needed
    logger.info("Attempting to load the latest state file.")
    ...
    if latest_state_file:
        ...
        logger.error("No existing state files found.")
        console.print(f"[bold red]Error:[/bold red] No simulation state files found in '{STATE_DIR}'.")
        console.print("Please run 'setup_simulation.py' first.")
        sys.exit(1)
    ...
    try:
        ...
        if not uuid_from_state or uuid_from_state != world_instance_uuid:
             logger.critical(f"UUID mismatch or missing in state file {state_file_path}.")
             console.print(f"[bold red]Error:[/bold red] UUID mismatch or missing in state file.")
             sys.exit(1)
        ...
        state = loaded_state_data # Assign loaded data to global state
    except Exception as e:
    ...
    state_modified = ensure_state_structure(state) # Pass global state
    if state_modified:
        logger.info("State structure updated. Saving state file.")
        try:
            save_json_file(state_file_path, state) # Save global state
    ...
    # --- Verify Simulacra (Integrated from initialize_simulation) ---
    console.rule("[cyan]Verifying Simulacra & Populating State[/cyan]")
    ...
    state["simulacra"] = state.get("simulacra", {}) # Ensure 'simulacra' key exists
    ...
    for sim_id in verified_active_sim_ids:
        ...
        if persona:
            ...
            if not current_location:
                loc_keys = list(state.get(WORLD_STATE_KEY, {}).get(LOCATION_DETAILS_KEY, {}).keys())
                primary_loc_key = loc_keys[0] if loc_keys else "UnknownLocation"
                current_location = home_location or primary_loc_key
                logger.warning(f"Simulacrum '{sim_id}' missing current location. Setting to '{current_location}'.")
            ...
            # Populate the main 'simulacra' state section
            ...
            logger.info(f"Populated state for simulacrum: {sim_id}")
        else:
    ...
    # --- Initialize ADK Components (Global Scope - Like Toy Example) ---
    ...
    adk_session = adk_session_service.create_session(
        ...
        state=state # <<< Pass the fully loaded global state here
    )
    logger.info(f"ADK Session created: {adk_session_id} with initial state.")

    # --- Instantiate Agents (Global Scope - Like Toy Example) ---
    ...
    # --- Create Runner (Global Scope - Like Toy Example) ---
    # Initialize with a default agent (e.g., world engine)
    world_engine_runner = Runner(
        agent=world_engine_agent,
        app_name=APP_NAME,
    ...
    try:
        ...
        else: logger.warning("No tasks list found or empty during cleanup.")
        logger.info("All tasks cancelled or finished.")

        # --- Save Final State ---
        final_uuid = state.get("world_instance_uuid") # Use global state
        if final_uuid:
            ...
            try:
                if not isinstance(state.get("world_time"), (int, float)):
                     logger.warning(f"Final world_time is not a number. Saving as 0.0.")
                     state["world_time"] = 0.0
                save_json_file(final_state_path, state) # Save global state
             ...
             logger.error("Cannot save final state: world_instance_uuid is not defined in global state.")
             console.print("[bold red]Error: Cannot save final state (UUID unknown).[/bold red]")

        console.print("\nFinal State Table:")
        if state: console.print(generate_table()) # Use global state
        else: console.print("[yellow]State dictionary is empty.[/yellow]")
...
if __name__ == "__main__":
    try:
        ...
        asyncio.run(run_simulation_main())
