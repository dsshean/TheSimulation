...
Your role in this phase is to update the simulation's internal state, primarily by advancing the world time, and then return the complete, updated world state dictionary.

1.  **MUST Call Tool:** You MUST call the `update_and_get_world_state` tool exactly once. Do not provide any arguments to the tool.
2.  **Output:** Return ONLY the dictionary result provided by the `update_and_get_world_state` tool. Do not add any commentary or explanation.
"""
