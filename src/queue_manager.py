...
class SequencedEventBus:
    ...
    async def put(self, event: Dict[str, Any]) -> None:
        """
        Backward compatibility method - mimics asyncio.Queue.put()
        """
        await self.put_event(event, timeout=5.0, task_name="LegacyPut")
    
    async def get(self) -> Dict[str, Any]:
        """
        Backward compatibility method - mimics asyncio.Queue.get()
        """
        result = await self.get_event(timeout=10.0, task_name="LegacyGet")
        if result is None:
